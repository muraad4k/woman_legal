...
WHERE doctors.verified='YES' AND schedules.available='yes' AND doctors.dr_id='$dr_id'";
    ...
    public function ReadProblemsUser($conn)
    {
        ...
        session_start();
        $id =$_SESSION['email'];
        $sql = "SELECT problem.file as file, problem.cus_id as caseId,user_login.user_id, problem.field_2 as caseType, user_login.email as userEmail,user_login.mobile as mobile,
    ...
    //   $name=$_SESSION['name'];
      // $result = mysqli_query($conn,"SELECT * FROM `requests` WHERE advocate='$name';");
        ...
        $sql="SELECT 
           ...
           FROM  requests
           JOIN 
               problem  ON requests.cus_id = problem.cus_id";
           $result = $conn->query($sql);
           if($result){
            $result = $conn->query($sql);
            if($result){
    ...
    public function createUser($conn)
    {
        ...
        if (move_uploaded_file($temp, $uploadedPath)) {
            move_uploaded_file($temp1,$uploadedPath1);
    ...
    public function createAdvocate($conn)
    {
        extract($_POST);
        $response = array();
        ...
        // echo $newName;
        if (move_uploaded_file($temp, $uploadedPath)) {
            $sql = "INSERT INTO `login`( `name`, `email`, `password`, `mobile`, `field_1`, `field_2`,`field_3`,`image`) VALUES ('$name','$email','$pass','$mobile','$city','$address','$id','$newName')";
            if (!$conn) {
                $response = array("error" => "there is an error connection", "status" => false,"c"=>$conn->error);
            } else {
                $result = $conn->query($sql);
                if ($result) {
                    $response = array("message" => "profile successfully created...", "status" => true);
                } else {
                    $response = array("error" => " error connection", "Status" => false,"c"=>$conn->error);
                }
            }
        }
    ...
    private function registerProblem($conn): bool{
        ...
        $actualFileName =rand().".".$exten;
        $tem = $_FILES['file']['tmp_name'];
        $fileUploaded = Doctors::uploadProblem($tem,$actualFileName);
        $sql = "INSERT INTO `problem`(`field_1`, `field_2`, `field_3`, `field_4`, `field_5`, `field_6`,`file`) VALUES ('$name','$case','$problem','$cnumber','$address','$mobile','$fileUploaded')";
        if (!$conn) {
           return false;
        } else {
            $result = $conn->query($sql);
    ...
    private function uploadProblem($temFolder, $file){
        if(!empty($file)){
            $uploadedPath = "./uploads/".$file;
            if(move_uploaded_file($temFolder, $uploadedPath))
               return $file;
            else 
             return "NoFile";
        }
    ...
    public function createProfile($conn)
    {
        ...
        if (move_uploaded_file($temp, $uploadedPath)) {
            move_uploaded_file($temp1,$uploadedPath1);
            $sql = "INSERT INTO `advocate`(`email`, `field_1`, `field_2`, `field_3`, `field_4`, `field_5`, `field_6`, `field_7`, `field_8`,`field_10`,`field_13`,`image`)
            VALUES(	'$email','$name','$field_2', 
                    '$field_3', '$field_4', '$field_5','$field_6',
                    '$field_7','$mobile','$field_9','$newName','$newName1')";
            if (!$conn) {
                $response = array("error" => "there is an error connection", "status" => false);
            } else {
                $result = $conn->query($sql);
    ...
    public function countActive($conn)
    {
        extract($_POST);
        $res = array();
        $sql = "SELECT COUNT(*) as counter from $table where status = '$status'";
        if (!$conn)
            $res = array("error" => "there is an error");
        else {
            $result = $conn->query($sql);
    ...
    public function countAdvocate($conn)
    {
        extract($_POST);
        $res = array();
        $sql = "SELECT COUNT(*) as counter from $table where field_9 = '$status'";
        if (!$conn)
            $res = array("error" => "there is an error");
        else {
            $result = $conn->query($sql);
    ...
    public function countAdvocateCases($conn)
    {
        extract($_POST);
        $res = array();
        session_start();
        // $name=$SESSION['ADVOCATE'];
        $sql = "SELECT COUNT(*) as counter from $table where status = '$status' and advocate='$name'";
        if (!$conn)
            $res = array("error" => "there is an error");
        else {
            $result = $conn->query($sql);
    ...
    public function countUserCases($conn)
    {
        extract($_POST);
        $res = array();
        session_start();
        // $name=$SESSION['ADVOCATE'];
        $sql = "SELECT COUNT(*) as counter from $table where status = '$status' and email='$email'";
        if (!$conn)
            $res = array("error" => "there is an error");
        else {
            $result = $conn->query($sql);
    ...
    public function countCases($conn)
    {
        extract($_POST);
        $res = array();
        session_start();
        // $name=$SESSION['ADVOCATE'];
        $sql = "SELECT COUNT(*) as counter from $table where  advocate='$name'";
        if (!$conn)
            $res = array("error" => "there is an error");
        else {
            $result = $conn->query($sql);
    ...
    public function user($conn)
    {
        ...
        session_start();
        // $name=$SESSION['ADVOCATE'];
        $sql = "SELECT COUNT(*) as counter from $table where  email='$email'";
        if (!$conn)
            $res = array("error" => "there is an error");
        else {
            $result = $conn->query($sql);
