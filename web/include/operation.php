...
function readpaid(){
  ...
  $array_data=array();
  $conn= new mysqli("localhost","root","","women_legal");
     $sql = "SELECT name,email,mobile,field_1,field_2 ,status,created_date from `user_login` WHERE `status`='$fee' ";   
     $result = $conn->query($sql);
     if($result){
      $result = $conn->query($sql);
      if($result){
...
function readpay(){
  ...
  if($table=='advocate'){
    ...
    $sql = "SELECT name,email,mobile,field_1,field_2 ,status,created_date from user_login WHERE   created_date BETWEEN '$FROM' AND '$TO' ";   
    $result = $conn->query($sql);
    if($result){
     $result = $conn->query($sql);
     if($result){
...
function readReport(){
  ...
  $conn= new mysqli("localhost","root","","women_legal");
     $sql = "SELECT name,email,mobile,field_1,field_2 ,status,created_date from `user_login`;";
     $result = $conn->query($sql);
     if($result){
      while($row=$result->fetch_assoc()){
        $array_data []=$row;
